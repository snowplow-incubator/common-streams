...
import software.amazon.kinesis.coordinator.{CoordinatorConfig, Scheduler, WorkerStateChangeListener}
...
private[source] object KCLScheduler {
    ...
    Sync[F].delay {
      ...
      val leaseManagementConfig =
        configsBuilder.leaseManagementConfig
          ...
          .leaseAssignmentIntervalMillis(kinesisConfig.leaseAssignmentInterval.toMillis)
          .gracefulLeaseHandoffConfig(
            LeaseManagementConfig.GracefulLeaseHandoffConfig
              .builder()
              .isGracefulLeaseHandoffEnabled(true)
              .gracefulLeaseHandoffTimeoutMillis(kinesisConfig.gracefulLeaseHandoffTimeout.toMillis)
              .build()
          )
          .workerUtilizationAwareAssignmentConfig(
            configsBuilder.leaseManagementConfig.workerUtilizationAwareAssignmentConfig().disableWorkerMetrics(true)
          )
  ...
  // We use shutdown() (immediate) instead of startGracefulShutdown() because by the time this
  // finalizer runs, the stream consumer fiber is already cancelled and nobody is draining the
  // action queue. startGracefulShutdown() would call shutdownRequested() on each shard processor,
  // which blocks on a CountDownLatch that never gets released, causing the pod to hang until KCL's
  // internal timeout fires. The LeaderDecider uninitialized error during shutdown() is harmless
  // noise — KCL catches it in its run-loop and exits within a second.
   ...
   *
   * Ignored once KCL transitions to 3.x lease assignment. With
   * CLIENT_VERSION_CONFIG_COMPATIBLE_WITH_2X, this is used until all workers are detected on 3.x
   * (~10 min), after which the leader-based assignment takes over and this setting has no effect.
   */
