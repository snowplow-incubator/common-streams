 ...
 * @param gracefulLeaseHandoffTimeout
 *   Minimum time to wait for the RecordProcessor's graceful shutdown before forcefully transferring
 *   the lease to the next owner. Increase for windowed apps to minimize duplicates.
 * @param leaseAssignmentInterval
 *   How frequently the KCL's LeaseAssignmentManager re-evaluates lease distribution across workers.
 *   Reducing this value minimizes the gap in record processing during lease transfers between
 *   workers.
 ...
 *   increase how aggressively it acquires leases. Ignored once KCL transitions from 2.x-compatible
 *   mode to 3.x leader-based lease assignment (typically ~10 min after all workers are on 3.x).
...
case class KinesisSourceConfig(
  ...
  gracefulLeaseHandoffTimeout: FiniteDuration,
  leaseAssignmentInterval: FiniteDuration,
