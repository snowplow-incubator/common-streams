...
import software.amazon.kinesis.lifecycle.events.{ProcessRecordsInput, ShardEndedInput, ShutdownRequestedInput}
...
private object KCLAction {
  ...
  /**
   * The action emitted by the ShardRecordProcessor during graceful lease handoff.
   *
   * @param await
   *   A countdown latch used to block the ShardRecordProcessor until all records from this shard
   *   have been checkpointed. Must only be released by the checkpointer.
   */
  final case class ShutdownRequested(
    shardId: String,
    await: CountDownLatch,
