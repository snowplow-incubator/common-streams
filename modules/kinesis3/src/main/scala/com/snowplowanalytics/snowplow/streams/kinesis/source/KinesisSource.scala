...
private[kinesis] object KinesisSource {
    ...
    Pull.eval(pullFromQueue(queue)).flatMap { actions =>
      val toEmit = actions.traverse {
        ...
        case KCLAction.ShardEnd(shardId, await, shardEndedInput) =>
          ...
          handleShardEnd[F](shardId, await, shardEndedInput.checkpointer)
        case KCLAction.ShutdownRequested(shardId, await, shutdownRequestedInput) =>
          // Do not call `await.countDown()` yet. It must be released later by the checkpointer.
          handleShardEnd[F](shardId, await, shutdownRequestedInput.checkpointer)
      ...
      val shouldEndWindow = actions.exists {
        case _: KCLAction.ShardEnd          => true
        case _: KCLAction.ShutdownRequested => true
      ...
      if (shouldEndWindow) {
        val log = Logger[F].info {
          actions
            .collect {
              case KCLAction.ShardEnd(shardId, _, _)          => s"$shardId (shard end)"
              case KCLAction.ShutdownRequested(shardId, _, _) => s"$shardId (shutdown requested)"
            }
            .mkString("Ending this window of events early for shards: ", ", ", "")
        }
  ...
  private def handleShardEnd[F[_]](
    ...
    val checkpointable = Checkpointable.ShardEnd(checkpointer, await)
