...
class KafkaSinkSpec extends Specification with CatsEffect {
  ...
  KafkaSink should:
    ...
    Report healthy initially (zero consecutive failures) $healthyInitially
    Report healthy after fewer than the threshold consecutive failures $healthyBelowThreshold
    Report unhealthy after reaching the consecutive failure threshold $unhealthyAtThreshold
    Report healthy again after a successful sink following failures $recoversAfterSuccess
  """
  ...
  // The threshold is private in KafkaSink, so we drive the test to it by counting failures.
  private val threshold = 5

  def healthyInitially = {
    val mock = newMock(true)
    KafkaSink.resourceWithProducer[IO](config, mock).use { sink =>
      sink.healthReporter
    } map { _ must beNone }
  ...
  def healthyBelowThreshold = {
    val failCount = threshold - 1
    KafkaSink.resourceWithProducer[IO](config, testProducer(Some(new RuntimeException("broker error")))).use { sink =>
      sink.sink(batch(1)).attempt.replicateA_(failCount) >>
        sink.healthReporter
    } map { _ must beNone }
  }

  def unhealthyAtThreshold = {
    KafkaSink.resourceWithProducer[IO](config, testProducer(Some(new RuntimeException("broker error")))).use { sink =>
      sink.sink(batch(1)).attempt.replicateA_(threshold) >>
        sink.healthReporter
    } map { _ must beSome }
  }

  def recoversAfterSuccess = {
    // Use an AtomicBoolean to control the producer without needing IO inside send()
    val shouldFail = new java.util.concurrent.atomic.AtomicBoolean(true)
    val brokerError = new RuntimeException("broker error")
    val switchableProducer: Producer[String, Array[Byte]] = new Producer[String, Array[Byte]] {
      def send(r: ProducerRecord[String, Array[Byte]]): JFuture[RecordMetadata] = {
        val f = new CompletableFuture[RecordMetadata]()
        if (shouldFail.get()) f.completeExceptionally(brokerError)
        else f.complete(new RecordMetadata(new TopicPartition(r.topic(), 0), 0L, 0, 0L, -1, -1))
        f
      }
      def send(r: ProducerRecord[String, Array[Byte]], cb: Callback): JFuture[RecordMetadata] = send(r)
      def flush(): Unit                                                                                              = ()
    ...
    KafkaSink.resourceWithProducer[IO](config, switchableProducer).use { sink =>
      for {
        _         <- sink.sink(batch(1)).attempt.replicateA_(threshold)
        unhealthy <- sink.healthReporter
        _         <- IO(shouldFail.set(false))
        _         <- sink.sink(batch(1))
        recovered <- sink.healthReporter
      } yield List(
        unhealthy must beSome,
        recovered must beNone
      ).reduce(_ and _)
    }
  }
}
