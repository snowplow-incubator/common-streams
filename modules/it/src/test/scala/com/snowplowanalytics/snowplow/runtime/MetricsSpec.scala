...
class MetricsSpec extends CatsResource[IO, (GenericContainer[_], StatsdAPI[IO])] with SpecificationLike {
  ...
  def e1 = withResource { case (statsd @ _, statsdApi) =>
    for {
      ...
      _ <- IO.sleep(190.millis)
      _ <- f.cancel
      counters <- statsdApi.get(Metrics.MetricType.Count)
      gauges <- statsdApi.get(Metrics.MetricType.Gauge)
    } yield List(
      counters.get("statsd.metrics_received") must beSome((x: Int) => x >= 1),
