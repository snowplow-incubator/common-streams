  ...
  private def startSubscriber[F[_]: Sync](
    ...
    Resource.make {
      Sync[F].delay {
        ...
        subscriber.setFailedMessageHandler { (_: String, _: String, msg: Message) =>
          dispatcher.unsafeRunAndForget(
            Logger[F].debug(s"Failed to process following message after multiple attempts: $msg") >>
              Sync[F].raiseError(new Exception(s"Failed to process message after multiple attempts"))
          )
        }
